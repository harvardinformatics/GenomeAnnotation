...
methodlabels<-c("BRAKER2","BRAKER1","BRAKER3",expression(CGP[protein]),expression(CGP[RNA]),expression(Scallop[HISAT2]),expression(Scallop[STAR]),expression(Stringtie[HISAT2]),expression(Stringtie[STAR]),"MAKER","TOGA","Liftoff")
...
refs_covstackedbar<- covlong %>% filter(species %in% c("D. melanogaster","H. sapiens","Z. mays","A. thaliana","G. gallus")) %>% ggplot(aes(x=method,y=fracpredprot*100,fill=interaction(methodtype,covclass))) +
            ...
            theme_classic() +
            theme(axis.text.x=element_text(angle=90, hjust=1,vjust=0.55,size=10,face="plain")) + 
            theme(strip.text = element_text(face="italic")) +
            theme(legend.text.align = 0,legend.text=element_text(size=10),legend.spacing.y = unit(0.1, "cm"),legend.key.size= unit(0.45, "cm")) + 
            ...
            labs(fill = "Reference protein\ncoverage by\nmethod type") +
            scale_x_discrete(labels=methodlabels) +
            xlab("") +
            theme(legend.position="bottom",legend.box.margin = margin(t = -25)) +

        

            facet_wrap(~species,ncol=3,nrow=2)
refs_covstackedbar
ggsave(refs_covstackedbar,file="Figure4.pdf",width=7,height=5)   
```
