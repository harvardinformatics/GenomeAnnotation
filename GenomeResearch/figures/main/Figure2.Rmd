...
subtitle: "Figure 2: CDS number-length biplots"
date: "April 10, 2024"
...
# Figure 2A: mammals 
```{r}
mammals_count_length_ratio_plot<- data %>% filter(group=="mammals") %>% ggplot() + 
...
print(mammals_count_length_ratio_plot)
ggsave(mammals_count_length_ratio_plot,file="Figure_2A.pdf",width=5,height=4) 
```

# Figure 2B: monocots
```{r}
monocots_count_length_ratio_plot<- data %>% filter(group=="monocots") %>% ggplot() + 
    geom_point(mapping=aes(y=number_CDS/protcod_mRNA_NCBI,x=MedianCDSLength/MedianCDSLength_ncbi,color=method,shape=species),alpha=0.7,size=2) + 
    facet_wrap(vars(group)) + ylab("# predicted CDS/# NCBI CDS") +
    geom_hline(yintercept=1,linetype="dotted",size=0.75) + 
    geom_vline(xintercept=1,linetype="dotted",size=0.75) +
...
print(monocots_count_length_ratio_plot)
ggsave(monocots_count_length_ratio_plot,file="Figure_2B.pdf",width=5,height=4) 
```
