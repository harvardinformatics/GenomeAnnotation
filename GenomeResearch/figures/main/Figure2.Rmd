...
refratio_biplot<- annotations %>% filter(species %in% c("H. sapiens","G. gallus","A. thaliana","D. melanogaster","Z. mays","M. musculus")) %>% ggplot() + 
    ...
    scale_color_manual(labels=methodlabels,values=c("yellow3","orange","brown4", "red","coral3", "lightskyblue", "royalblue3","darkorchid3", "magenta","gray72","green3","seagreen")) +
     ...
     theme(axis.title=element_text(size=12),legend.title = element_text(size = 12),legend.text.align = 0,legend.spacing.y = unit(0.15, "cm"),legend.key.size= unit(0.5, "cm")) + 
    guides(color = guide_legend(title = "Method",override.aes = list(shape=15),legend.title=element_text(size=12),order=1,label.theme = element_text(angle = 0, size=11,face = "plain"))) +
    guides(shape = guide_legend(title = "Species",legend.title=element_text(size=12),order=2,label.theme = element_text(angle = 0, size=12,face = "italic"))) +
  theme(axis.text.y=element_text(size=11)) +
  theme(axis.text.x=element_text(size=11)) +
  
  geom_vline(xintercept=1,linetype="dotted",linewidth=1,color="black") +
