...
subtitle: "Figure 7: RNA-seq alignment rate to transcriptome: 
...
## Figure 7
```{r}
transcriptome_alignmentrate_plot <- annotations %>% filter(group!="heliconines") %>% ggplot(aes(x=species,y=median_align_splicedexons)) + 
geom_point(aes(x=species,y=RefTsAlignRateAll),shape=4,color="black",size=3.5) +
geom_point(aes(fill=method),color="black",pch=21,position = position_jitter(w = 0.3,h = 0),alpha=0.8,size=2.6) +
...
ylab(expression(paste(RNA[seq]," alignment rate to transcriptome"))) +
xlab("") + 
...
geom_vline(xintercept=18.5,linetype="solid",size=1) +
geom_vline(xintercept=17.5,linetype="dotted",size=0.2) +
geom_vline(xintercept=19.5,linetype="dotted",size=0.2) +
geom_vline(xintercept=20.5,linetype="dotted",size=0.2) +
guides(fill = guide_legend(title = "")) 


print(transcriptome_alignmentrate_plot)
ggsave(transcriptome_alignmentrate_plot,file="Figure7.pdf",width=8.5,height=5)
```
