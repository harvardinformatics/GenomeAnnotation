...
subtitle: "Figure 6: transcriptome vs. proteome compleasm scores: 
...
## Figure 6
```{r}
refs_plot <- annotations %>% filter(species %in% c("D. melanogaster","H. sapiens","Z. mays","A. thaliana","G. gallus")) %>% 
      ggplot() + 
    ...
    theme(legend.text.align = 0,axis.title=element_text(size=12),legend.title = element_text(size = 12),legend.spacing.y = unit(0.15, "cm"),legend.key.size= unit(0.5, "cm")) + 
    guides(fill = guide_legend(title = "Method",override.aes = list(shape=21),legend.title=element_text(size=12),order=1,label.theme = element_text(angle = 0, size=11,face = "plain"))) +
    guides(shape = guide_legend(title = "Species",order=2,legend.title=element_text(size=12),label.theme = element_text(angle = 0, size=11,face = "italic"))) +
    theme(axis.text.y=element_text(size=11)) +
  theme(axis.text.x=element_text(size=11)) 
    

print(refs_plot)
ggsave(refs_plot,file="Figure6.pdf",width=4.5,height=4.5)
```
