...
subtitle: "Figure S9: RNA-seq alignment rate to proteome (CDS): 
...
## Figure S9
```{r}
cds_alignmentrate_plot <- annotations %>% filter(group!="heliconines") %>% ggplot(aes(x=species,y=median_annot_align_noutr)) + 
geom_point(aes(fill=method),color="black",pch=21,position = position_jitter(w = 0.33,h = 0),alpha=0.7,size=2.8) +
...
ylab(expression(paste(RNA[seq]," alignment rate to CDS"))) +
xlab("") + 
...
print(cds_alignmentrate_plot)
ggsave(cds_alignmentrate_plot,file="Figure_S9.pdf",width=8,height=5.5)
```
