...
subtitle: "Figure S5: reference protein coverage by taxonomic group
...
## Figure S5: panel A, vertebrates
```{r}
verts_covstackedbar<- covlong %>% filter(group %in% c("birds","mammals")) %>% 
            ggplot(aes(x=method,y=fracpredprot*100,fill=interaction(methodtype,covclass))) +
...
verts_covstackedbar
ggsave(verts_covstackedbar,file="Figure_S5A.pdf",width=8,height=8)
```

## Figure S5: panel B, plants
```{r}
plants_covstackedbar<- covlong %>% filter(group %in% c("rosids","monocots")) %>% 
           ggplot(aes(x=method,y=fracpredprot*100,fill=interaction(methodtype,covclass))) +
            geom_bar(width=0.9,stat="identity",position="stack") +
            scale_fill_manual(breaks=c("braker.proref_blastcovered0.8","braker.proref_lowcov","cgp.proref_blastcovered0.8","cgp.proref_lowcov","assembler.proref_blastcovered0.8","assembler.proref_lowcov",
                    "maker.proref_blastcovered0.8", "maker.proref_lowcov",
...
plants_covstackedbar
ggsave(plants_covstackedbar,file="Figure_S5B.pdf",width=8,height=8)
```

## Figure S5: panel C, dipterans
```{r}
dipterans_covstackedbar<- covlong %>% filter(group %in% c("dipterans")) %>% 
           ggplot(aes(x=method,y=fracpredprot*100,fill=interaction(methodtype,covclass))) +
            geom_bar(width=0.9,stat="identity",position="stack") +
           scale_fill_manual(breaks=c("braker.proref_blastcovered0.8","braker.proref_lowcov","cgp.proref_blastcovered0.8","cgp.proref_lowcov","assembler.proref_blastcovered0.8","assembler.proref_lowcov",
                    "maker.proref_blastcovered0.8", "maker.proref_lowcov",
...
dipterans_covstackedbar
ggsave(dipterans_covstackedbar,file="Figure_S5C.pdf",width=8,height=4)
```
